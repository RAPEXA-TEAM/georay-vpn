package com.example.cross_vpn.v2ray.viewmodel
...
import com.example.cross_vpn.MainActivity
import com.example.cross_vpn.R
import com.google.gson.Gson
import com.tencent.mmkv.MMKV
import com.example.cross_vpn.v2ray.AppConfig
import com.example.cross_vpn.v2ray.AppConfig.ANG_PACKAGE
import com.example.cross_vpn.v2ray.dto.*
import com.example.cross_vpn.v2ray.util.*
import com.example.cross_vpn.v2ray.util.MmkvManager.KEY_ANG_CONFIGS
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun startListenBroadcast() {
        ...
        getApplication<CrossVpnApp>().registerReceiver(mMsgReceiver, IntentFilter(AppConfig.BROADCAST_ACTION_ACTIVITY))
        MessageUtil.sendMsg2Service(getApplication(), AppConfig.MSG_REGISTER_CLIENT, "")
    }

    override fun onCleared() {
        getApplication<CrossVpnApp>().unregisterReceiver(mMsgReceiver)
...
//        getApplication<CrossVpnApp>().toast(R.string.connection_test_testing)
        ...
        override fun onReceive(ctx: Context?, intent: Intent?) {
            when (intent?.getIntExtra("key", 0)) {
                AppConfig.MSG_STATE_RUNNING -> {
                    ...
                    MainActivity.globalEventSinkV2ray?.success(AppConfig.MSG_STATE_RUNNING)
                }
                AppConfig.MSG_STATE_NOT_RUNNING -> {
                    isRunning.value = false
                }
                AppConfig.MSG_STATE_START_SUCCESS -> {
//                    getApplication<CrossVpnApp>().toast(R.string.toast_services_success)
                    isRunning.value = true
                    MainActivity.globalEventSinkV2ray?.success(AppConfig.MSG_STATE_START_SUCCESS)
                }
                AppConfig.MSG_STATE_START_FAILURE -> {
//                    getApplication<CrossVpnApp>().toast(R.string.toast_services_failure)
                    isRunning.value = false
                    MainActivity.globalEventSinkV2ray?.success(AppConfig.MSG_STATE_START_FAILURE)
                }
                AppConfig.MSG_STATE_STOP_SUCCESS -> {
                    isRunning.value = false
                    MainActivity.globalEventSinkV2ray?.success(AppConfig.MSG_STATE_STOP_SUCCESS)
                }
                AppConfig.MSG_MEASURE_DELAY_SUCCESS -> {
                    updateTestResultAction.value = intent.getStringExtra("content")
                    MainActivity.globalEventSinkV2ray?.success(AppConfig.MSG_MEASURE_DELAY_SUCCESS)
                }
                AppConfig.MSG_MEASURE_CONFIG_SUCCESS -> {
                    val resultPair = intent.getSerializableExtra("content") as Pair<String, Long>
                    MmkvManager.encodeServerTestDelayMillis(resultPair.first, resultPair.second)
                    updateListAction.value = getPosition(resultPair.first)
                    MainActivity.globalEventSinkV2ray?.success(AppConfig.MSG_MEASURE_CONFIG_SUCCESS)
                }
            }
        }
    }
