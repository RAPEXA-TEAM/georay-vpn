...
import com.example.cross_vpn.v2ray.AppConfig.TAG_DIRECT
import com.example.cross_vpn.R
import com.example.cross_vpn.v2ray.dto.ServerConfig
import com.example.cross_vpn.v2ray.extension.toSpeedString
...
import com.example.cross_vpn.v2ray.util.V2rayConfigUtil
import go.Seq
    ...
    fun startV2rayPoint() {
        ...
        if (!v2rayPoint.isRunning) {
            ...
            if (v2rayPoint.isRunning) {
                ...
                mainStorage.encode(MmkvManager.VPN_CONNECTED, true)
                showNotification()
            } else {
                MessageUtil.sendMsg2UI(service, AppConfig.MSG_STATE_START_FAILURE, "")
                mainStorage.encode(MmkvManager.VPN_CONNECTED, false)
                cancelNotification()
            }
        }
    }
    ...
    private fun updateNotification(contentText: String?, proxyTraffic: Long, directTraffic: Long) {
        if (mBuilder != null) {
            if (proxyTraffic < NOTIFICATION_ICON_THRESHOLD && directTraffic < NOTIFICATION_ICON_THRESHOLD) {
                mBuilder?.setSmallIcon(R.drawable.ic_stat_vpn)
