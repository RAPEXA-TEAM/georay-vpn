...
import com.example.cross_vpn.v2ray.AppConfig.ANG_CONFIG
import com.example.cross_vpn.v2ray.AppConfig.HTTPS_PROTOCOL
import com.example.cross_vpn.v2ray.AppConfig.HTTP_PROTOCOL
import com.example.cross_vpn.R
import com.example.cross_vpn.openconnect.VpnConstant
import com.example.cross_vpn.v2ray.dto.*
import com.example.cross_vpn.v2ray.dto.V2rayConfig.Companion.DEFAULT_SECURITY
import com.example.cross_vpn.v2ray.dto.V2rayConfig.Companion.TLS
import com.example.cross_vpn.v2ray.util.MmkvManager.KEY_SELECTED_SERVER
import java.net.URI
import java.util.*
import com.example.cross_vpn.v2ray.extension.idnHost
...
object AngConfigManager {
    ...
    private fun migrateVmessBean(angConfig: AngConfig, sharedPreferences: SharedPreferences) {
        angConfig.vmess.forEachIndexed { index, vmessBean ->
            ...
            config.remarks = "Connected"+ VpnConstant.countryNameV2ray
    ...
    private fun migrateSubItemBean(angConfig: AngConfig) {
        angConfig.subItem.forEach {
            ...
            subItem.remarks = VpnConstant.countryNameV2ray
            subItem.url = it.url
    ...
    fun getServerConfigFromStr(str: String): ServerConfig? {
        var config: ServerConfig? = null
        try {
            val allowInsecure = settingsStorage?.decodeBool(AppConfig.PREF_ALLOW_INSECURE) ?: false
            if (str.startsWith(EConfigType.VMESS.protocolScheme)) {
