...
public class ProfileManager {
	...
	public static String makeProfName(String s, int index) {
		String orig = s;
		String suffix;
	...
	public synchronized static VpnProfile createStr(String hostname) {

		String uuid = UUID.randomUUID().toString();
		SharedPreferences p = mContext.getSharedPreferences(getPrefsName(uuid), Activity.MODE_PRIVATE);
		p.edit().putString("server_address", hostname).commit();

		VpnProfile profile = new VpnProfile(p, uuid, hostname);
		mProfiles.put(uuid, profile);
		return profile;
	}
